...
class NiftiToNrrdConverter(QWidget):
    ...
    def get_overlay_image(self, base, roi):
        ...
        overlay[..., 0] = 255       # Red
        overlay[..., 3] = roi * 100  # Transparency mask
        return overlay
    ...
    def update_views(self):
        ...
        if self.nrrd_data is not None:
            ...
            self.show_image_with_overlay(self.nrrd_viewer, nrrd_vol, self.roi_data, "nrrd")

        if self.nifti_data is not None:
            nifti_vol = self.apply_orientation(self.nifti_data)
            self.show_image_with_overlay(self.nifti_viewer, nifti_vol, self.roi_data, "nifti")
